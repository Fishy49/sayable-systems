...
/** Every raw row still held for this profile, oldest first. */
export async function eventsCsv(profileId: string): Promise<string> {
  const rows = (await usageRange()).filter((r) => r.profileId === profileId);
  return csvRows([
    ['when', 'kind', 'text', 'words_in_sentence'],
    ...rows.map((r) => [new Date(r.ts).toISOString(), r.kind, r.text, r.words ?? '']),
  ]);
}

/** The permanent weekly history, including weeks whose raw rows are long gone. */
export async function weeksCsv(profileId: string): Promise<string> {
  const weeks = (await usageWeeksAll()).filter((w) => w.profileId === profileId);
  return csvRows([
    ['week_starting', 'word_taps', 'sentences_spoken', 'mean_sentence_length', 'unique_words', 'active_days'],
    ...weeks.map((w) => [
      new Date(w.weekStart).toISOString().slice(0, 10),
      w.words,
      w.utterances,
      w.utterances ? (w.utteranceWords / w.utterances).toFixed(2) : '',
      w.uniqueWords,
      w.activeDays,
    ]),
  ]);
...
export async function hasAnyUsageData(): Promise<boolean> {
  try {
    const [raw, weeks] = await Promise.all([usageRange(), usageWeeksAll()]);
    return raw.length > 0 || weeks.length > 0;
  } catch {
    return false;
  }
}
